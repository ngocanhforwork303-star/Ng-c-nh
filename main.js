...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const slides = document.querySelectorAll('.s-about__block');
    
    // Nếu trang nào không có slider thì bỏ qua để không báo lỗi
    ...
    function updateSlider() {
        const slideWidth = slides[0].offsetWidth + 30; // 30 là khoảng cách margin
    ...
    // Dừng tự động chạy khi người dùng đưa chuột vào
    const sliderWrap = document.querySelector('.s-about-slider-wrap');
    if(sliderWrap) {
        sliderWrap.addEventListener('mouseenter', () => clearInterval(autoSlide));
        sliderWrap.addEventListener('mouseleave', () => autoSlide = setInterval(nextSlide, 3000));
    }
    ...
    window.addEventListener('resize', () => {
        currentIndex = 0; // Đưa về slide đầu tiên khi đổi kích thước
        updateSlider();
    });
});
